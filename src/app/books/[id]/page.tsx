...
async function page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const response = await fetch(`http://localhost:8000/api/v2/book/${id}`);
      ...
      <div className="grid grid-cols-4 w-full">
        <div className="col-span-2 p-4 ">
          ...
          <Image
            ...
            alt={bookInfo.title}
            className="lg:h-96 lg:w-70 ml-auto"
            height={100}
            width={100}
          />
        </div>
        <div className="col-span-2 flex flex-col items-start space-y-2 p-4">
          ...
          <p className="text-base">
            Publish Date: {bookInfo.published_at.toString()}
          </p>
            ...
            <div className="flex flex-col space-y-2">
              ...
              <Button variant="default">
                <Link href="" className="flex gap-3 group">
                  <ShoppingCart className="group-hover:fill-white translate-y-1" />{" "}
                  Buy now
                </Link>
              </Button>
            </div>
